...
\vspace{-.2in}
\item[] Engineering Manager with 9 years of experience in the auotonomous driving industry.
...
% \hspace{-1em} Languages/Libraries/Tools & PyTorch, Python, C/C$+$$+$\\
% \end{tabular}
% \end{rSection}
% \vspace{-.05in}
...
\item I built Latitude AI's Perception Applications team.
\item My team works on the perception capabilities for Ford's Universal Electric Vehicles (UEVs).
\end{rSubsection}
% \vspace{-.05in}
...
\item I built from scratch Motional's Prediction team, with 16 people including three TLs and one TLM.
\item My team owned the behavior prediction and ego trajectory generation models for Motional's self-driving car. The models use transformer and graph attention architectures to predict the other road users' future behaviors as well as generate future ego trajectories.
\end{rSubsection}
% \vspace{-.05in}
...
\item As a tech lead and a pioneer in DL-based behavior prediction, I owned Uber's production behavior prediction models and published three first-author papers at top conferences, including the MTP paper which is one of the default baseline methods in the Argoverse motoin forecasting challenge.
...
\vspace{-.1in}
    \item
    [1]
        EuroSys 2026 Test of Time Award  \hfill ACM EuroSys, 2026

        \footnotesize{
            Recognized for the lasting impact of ``GeePS: Scalable Deep Learning on Distributed GPUs with a GPU-Specialized Parameter Server'', originally published at EuroSys 2016.
        }
